...
public class WorkerScheduleExecutor {
    ...
    public OneTimeWorkRequest runPostLoginSync(User user) {
        ...
        workManager.beginUniqueWork("INITIAL_APP_SETUP", ExistingWorkPolicy.KEEP, userOneTimeWorkRequest).then(tutorOneTimeWorkRequest).then(tutoredOneTimeworkResquest)
                .then(programmaticAreaOneTimeworkResquest).then(formOneTimeworkResquest).enqueue();
        return formOneTimeworkResquest;
    }
}
